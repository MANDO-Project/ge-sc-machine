...
NODE_MODEL_OPTS = {BugType.ACCESS_CONTROL: node_classifier_access_control,
...
GRAPH_MODEL_OPTS = {BugType.ACCESS_CONTROL: graph_classifier_access_control,
...
@router.post('/{node_feature}', response_model=MultiBuggyNodeDetectReponse, dependencies=[Security(get_api_key)])
...
@router.post('/{node_feature}/test', response_model=MultiBuggyNodeDetectReponse, dependencies=[Security(get_api_key)])
async def extra_detect_line_level_bugs(data: ContractRequest,
                                       node_feature: NodeFeature = NodeFeature.NODE_TYPE):
