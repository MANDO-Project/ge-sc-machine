...
from ...config import settings_access_contro,settings_arithmetic,settings_denial_of_service,settings_front_running,settings_reentrancy,settings_time_manipulation,settings_unchecked_low_level_calls
...
is_gpu = check_gpu()
node_classifier_access_contro, smart_contracts = init_line_node_classificator(settings_access_contro.LINE.CHECKPOINT,
                                          settings_access_contro.LINE.COMPRESSED_GRAPH,
                                          settings_access_contro.LINE.DATASET,
                                          settings_access_contro.LINE.feature_extractor,
                                          settings_access_contro.DEVICE)
node_classifier_arithmetic, smart_contracts = init_line_node_classificator(settings_arithmetic.LINE.CHECKPOINT,
...
node_classifier_denial_of_service, smart_contracts = init_line_node_classificator(settings_denial_of_service.LINE.CHECKPOINT,
...
node_classifier_front_running, smart_contracts = init_line_node_classificator(settings_front_running.LINE.CHECKPOINT,
...
node_classifier_reentrancy, smart_contracts = init_line_node_classificator(settings_reentrancy.LINE.CHECKPOINT,
                                          settings_reentrancy.LINE.COMPRESSED_GRAPH,
                                          settings_reentrancy.LINE.DATASET,
...
async def detect_line_level_bugs(item: NodeRequest):
    ...
    node_type=get_node_type(node_classifier_unchecked_low_level_calls.nx_graph, [item.filename])
    with torch.no_grad():
...
@router.post('/line/access_contro', response_model=NodeResponse, dependencies=[Security(get_api_key)])
async def detect_line_level_bugs(item: NodeRequest):
    
    node_classifier_access_contro.eval()
    file_ids = get_node_ids(node_classifier_access_contro.nx_graph, [item.filename])
    line_numbers = get_line_numbers(node_classifier_access_contro.nx_graph, [item.filename])
    file_edges=get_edges(node_classifier_access_contro.nx_graph, [item.filename],file_ids)
    color_nodes=get_color_node(node_classifier_access_contro.nx_graph, [item.filename])
    node_type=get_node_type(node_classifier_access_contro.nx_graph, [item.filename])

    with torch.no_grad():
        logits = node_classifier_access_contro()
        logits = logits.to(settings_access_contro.DEVICE)
        file_mask = get_binary_mask(node_classifier_access_contro.total_nodes, file_ids)
        preds = logits[file_mask]
        _, indices = torch.max(preds, dim=1)
        preds = indices.long().cpu().tolist()
        assert len(preds) == len(line_numbers)
...
@router.post('/line/time_manipulation', response_model=NodeResponse, dependencies=[Security(get_api_key)])
async def detect_line_level_bugs(item: NodeRequest):
    
    node_classifier_time_manipulation.eval()
    file_ids = get_node_ids(node_classifier_time_manipulation.nx_graph, [item.filename])
    line_numbers = get_line_numbers(node_classifier_time_manipulation.nx_graph, [item.filename])
    file_edges=get_edges(node_classifier_time_manipulation.nx_graph, [item.filename],file_ids)
    color_nodes=get_color_node(node_classifier_time_manipulation.nx_graph, [item.filename])
    node_type=get_node_type(node_classifier_time_manipulation.nx_graph, [item.filename])

    with torch.no_grad():
        logits = node_classifier_time_manipulation()
        logits = logits.to(settings_time_manipulation.DEVICE)
        file_mask = get_binary_mask(node_classifier_time_manipulation.total_nodes, file_ids)
        preds = logits[file_mask]
        _, indices = torch.max(preds, dim=1)
        preds = indices.long().cpu().tolist()
        assert len(preds) == len(line_numbers)
